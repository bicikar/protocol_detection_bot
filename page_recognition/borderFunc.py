...
def line_intersection(x1, y1, x2, y2, x3, y3, x4, y4):
    ...
    if ((x1 >= x3 - 5 or x1 >= x3 + 5) and (x1 <= x4 + 5 or x1 <= x4 - 5) and (
            y3 + 8 >= min(y1, y2) or y3 - 5 >= min(y1, y2)) and y3 <= max(y1,
                                                                          y2) + 5):
        return x1, y3
...
def extract_table(table_body, __line__, lines=None):
    ...
    # for point in points:
    #     for x, y in point:
    #         cv2.line(table, (x, y), (x, y), (0, 0, 255), 15)
    #
    # cv2.imshow("intersection", cv2.resize(table, (1490, 1054)))
    # cv2.waitKey(0)
    ...
    #     cv2.rectangle(table_body, (i[0], i[1]), (i[6], i[7]), (0,0,255), 2)
    # #     count+=1
    # cv2.imshow("cells", table_body)
    # cv2.waitKey(0)
    ############################
    if len(temp_lines_ver) - 1 < 5:
        rows = 0
        cols = 0
    else:
        cols = len(temp_lines_ver) - 1
        rows = len(box) // cols
    return box, rows, cols


# print(extract_table(cv2.imread('tmp/images/img1.png'), 1, lines=None))
...
def extractText(img):
    ...
    for cnt in contours:
        ...
        # print(im2.shape)
        if x < 2 or y < 2 or (
                x + w >= im2.shape[1] - 1 and y + h >= im2.shape[0] - 1) or w >= \
                im2.shape[1] - 1 or h >= im2.shape[0] - 1:
            continue
        if x < mx:
            mx = x
        if y < my:
