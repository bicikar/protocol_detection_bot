...
def line_detection(image):
    ...
    hor_lines = cv2.HoughLinesP(horizontal, rho=1, theta=np.pi / 180,
                                threshold=100, minLineLength=30, maxLineGap=3)
    ...
    ver_lines = cv2.HoughLinesP(vertical, 1, np.pi / 180, 20, np.array([]), 20,
                                2)
    ...
    #             min(y1, y2) < min(lasty1, lasty2) - 20 or min(y1, y2) < min(
    #             lasty1, lasty2) + 20)) and ((
    #             max(y1, y2) < max(lasty1, lasty2) - 20 or max(y1, y2) < max(
    #             lasty1, lasty2) + 20))):
    #         lines_y1.append(y1)
    #         lines_y2.append(y2)
    #         lasty1 = y1
    ...
    #             ver.append(
    #                 [lastx1, min(lines_y2) - 5, lastx1, max(lines_y1) - 5])
    #         lastx1 = x1
    #         lines_y1 = []
    #         lines_y2 = []
    #         lines_y1.append(y1)
    ...
    for x1, y1, x2, y2 in ver_lines:
        if abs(x1 - lastx1) < 15:
            if abs(y2 - y1) - 40 > abs(lasty2 - lasty1):
    ...
    for x1, y1, x2, y2 in ver:
        max_ver = max(max_ver, abs(y2 - y1))
    ver = [line for line in ver if abs(line[3] - line[1]) > max_ver - 20]
    #################################################################
    hor = hor[1:]
    ...
    #     cv2.line(img, (x1,y1), (x2,y2), (0, 255, 0), 1)
    # #
    # cv2.imshow("image", cv2.resize(img, (1490, 1054)))
    # cv2.waitKey(0)
    #######################################################################
    return hor, ver